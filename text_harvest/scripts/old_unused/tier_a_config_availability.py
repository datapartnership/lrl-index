...
actually has a per-language HF config at all (either the bare code,
or a {code}_{Script}-style match), versus datasets that only expose a
single flat config (commonly "default") with no per-language split -
e.g. panlex, language_tags, mms_ulab_v2, discovered during
development of tier_a_harvest_v3.py, where language likely lives as a
DATA COLUMN inside one config rather than as separate configs.
...
script_suffixed_matches (any {code}_* matches), has_per_language_config
(exact_match OR script_suffixed_matches - the main yes/no this
experiment is measuring), likely_single_default_config (flag:
total_config_count == 1 AND that one config is "default" or matches
the dataset's own name - a strong signal language lives in the data,
not the config list).

--------------------------------------------------------------------
NOT A HARVEST STEP
--------------------------------------------------------------------
No row/hour counting, no size resolution, no README checking - purely
"does a per-language config exist, yes or no, and what does the real
config list look like when it doesn't."
...
def check_dataset_config_availability(dataset_id, hf_language_tag, hf_token):
    """
    Returns a row dict, or None if the config list itself couldn't be
    fetched at all after retries (skip, don't fabricate a row).
    """
    ...
    script_suffixed_matches = [c for c in all_configs if suffix_pattern.match(c)]
    has_match = exact_match or bool(script_suffixed_matches)
    ...
    return {
        ...
        "script_suffixed_matches": ";".join(script_suffixed_matches) if script_suffixed_matches else None,
...
def print_summary(output_path):
    ...
    print(f"Has per-language config (exact or script-suffixed): {has_match} ({has_match / total:.1%})")
    print(f"Likely single 'default' config (language probably a data column): {single_default} ({single_default / total:.1%})")
