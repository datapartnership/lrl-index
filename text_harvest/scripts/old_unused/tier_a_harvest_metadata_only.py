"""
tier_a_harvest_v5_metadata.py

COMPARISON VARIANT of tier_a_harvest_v5.py - identical in every way
EXCEPT how the row count is obtained. This version uses
load_dataset_builder(...).info.splits[split].num_examples - FREE, no
download, no data transfer - instead of v5's actual load_dataset()
call. Field is named num_examples throughout (not num_rows) to keep
the two scripts' outputs visually distinguishable.

See conversation for why this comparison run exists: v5's
load_dataset() approach took 10+ minutes on a single 3.76GB dataset,
raising real concerns about feasibility across a full crosswalk run
when some HF datasets are hundreds of GB to tens of TB. This variant
trades exactness (a dataset lacking precomputed metadata gets no
count at all here, vs. v5 which would always eventually get one via
download) for speed and zero data transfer.

--------------------------------------------------------------------
FIELDS RECORDED
--------------------------------------------------------------------
dataset_id, language_code, languages_in_dataset, linguality,
license, modality, tasks, num_examples, provenance, synthetic_flag,
...
--------------------------------------------------------------------
num_examples RESOLUTION (all FREE - no download, no data transfer)
--------------------------------------------------------------------
Monolingual: (a) check the dataset's OVERALL metadata (no config
specified) for num_examples first; (b) if not there, fall back to
checking the 'default' configuration specifically.
Multilingual: check for a per-language configuration (bare code, or
delimited anywhere in the config name - see match_language_configs)
and read num_examples from THAT configuration's precomputed metadata.
No per-language config = out of scope (goes to the catch-all output).
...
--------------------------------------------------------------------
FIVE OUTPUT FILES, MUTUALLY EXCLUSIVE (priority order below)
--------------------------------------------------------------------
1. FULL_CLEAN_FILE - num_examples successfully resolved AND provenance
   is NOT unclear.
2. MANUAL_REVIEW_FILE - (a) access issues (404-style), checked FIRST,
   OR (b) num_examples WAS successfully resolved but provenance is
   unclear.
3. NO_PRECOMPUTED_METADATA_FILE - a per-language config was found (or
   the dataset is monolingual/'default'), but this dataset simply
   doesn't ship precomputed num_examples metadata (dataset_infos.json /
   auto-Parquet-conversion stats absent) - most common for datasets
   using a legacy/custom loading script. UNLIKE v5, this is NOT
   resolvable by trying harder here - there's genuinely nothing free
   to read. v5's load_dataset() would still get an exact count for
   many of these (at the cost of a real download); this variant
   simply can't.
...
5. SKIPPED_NON_TEXT_FILE - modality is explicitly stated and doesn't
   include "text" - filtered before any further processing.
...
FULL_CLEAN_FILE = "../data/tier_a_v5_metadata/tier_a_v5_metadata_full_clean.csv"
MANUAL_REVIEW_FILE = "../data/tier_a_v5_metadata/tier_a_v5_metadata_manual_review.csv"
NO_PRECOMPUTED_METADATA_FILE = "../data/tier_a_v5_metadata/tier_a_v5_metadata_no_precomputed_metadata.csv"
EVERYTHING_ELSE_FILE = "../data/tier_a_v5_metadata/tier_a_v5_metadata_everything_else.csv"
SKIPPED_NON_TEXT_FILE = "../data/tier_a_v5_metadata/tier_a_v5_metadata_skipped_non_text_modality.csv"
...
def get_num_examples(dataset_id, config_name, hf_token):
    """
    Returns (num_examples_or_None, error_str_or_None). Uses
    load_dataset_builder(...).info.splits[split].num_examples - FREE,
    no download, no data transfer, just reads whatever precomputed
    metadata the dataset ships (dataset_infos.json / auto-generated
    Parquet-conversion stats). This is the comparison variant to
    tier_a_harvest_v5.py's get_num_examples(), which does an actual
    load_dataset() call instead - see conversation on the runtime
    tradeoff between the two (3.76GB dataset took 10+ minutes via
    load_dataset(); this version never downloads dataset content at
    all, but returns None whenever a dataset doesn't have this
    metadata precomputed, rather than an exact count via download).

    Sums num_examples across every split in the builder's info -
    matches the "total across all splits" behavior of the v5 script.
    """
    try:
    ...
    if not builder.info.splits:
        return None, None  # resolved fine, just no precomputed row info - not an error to report upstream
...
def process_one_dataset(dataset_id, hf_language_tag, hf_token):
    ...
    if info is None:
        return {
            ...
            "num_examples": None, "provenance": None, "synthetic_flag": None,
    ...
    # ---- Text-modality filter, checked as early as possible ----
    # Skip entirely if modality IS explicitly stated and "text" isn't
    # among the listed modalities - no further processing (README
    # fetch, config resolution, num_examples download) is worth doing on
    # a confirmed non-text dataset. If modality is UNSTATED (no tags
    # at all), still processed - untagged datasets are common on HF
    # and a large share of them are plain text that was just never
    # tagged; excluding all of them would likely drop real data.
    ...
    if linguality != "multilingual":
        num_examples, err = get_num_examples(dataset_id, None, hf_token)
        matched_config = "overall"
        if num_examples is None and not (err and classify_access_issue(err) != "other"):
            num_examples, err = get_num_examples(dataset_id, "default", hf_token)
            matched_config = "default"
        config_fields = {"matched_configurations": matched_config if num_examples is not None else None,
                          "all_configurations": None, "all_configurations_count": None}

        if err and classify_access_issue(err) != "other":
            return {**base_row, "num_examples": None, **config_fields,
                    "manual_review": "404 - need to grant access to dataset"}, "manual_review"

        if num_examples is not None:
            if provenance == "unclear":
                return {**base_row, "num_examples": num_examples, **config_fields,
                        "manual_review": "check unclear provenance"}, "manual_review"
            return {**base_row, "num_examples": num_examples, **config_fields, "manual_review": None}, "full_clean"

        return {**base_row, "num_examples": None, "matched_configurations": None,
                "all_configurations": None, "all_configurations_count": None,
    ...
    if config_err and classify_access_issue(config_err) != "other":
        return {**base_row, "num_examples": None,
                "matched_configurations": None, "all_configurations": None, "all_configurations_count": None,
                "manual_review": "404 - need to grant access to dataset"}, "manual_review"
    ...
    if not resolved_configs:
        return {**base_row, "num_examples": None, "matched_configurations": None, **config_fields,
                "manual_review": None}, "everything_else"

    total_rows = 0
    any_resolved = False
    for config_name in resolved_configs:
        num_examples, _ = get_num_examples(dataset_id, config_name, hf_token)
        if num_examples is not None:
    ...
    if any_resolved:
        if provenance == "unclear":
            return {**base_row, "num_examples": total_rows, **matched_fields,
                    "manual_review": "check unclear provenance"}, "manual_review"
        return {**base_row, "num_examples": total_rows, **matched_fields, "manual_review": None}, "full_clean"

    return {**base_row, "num_examples": None, **matched_fields, "manual_review": None}, "no_precomputed_metadata"
